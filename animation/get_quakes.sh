...
# You can edit the dates, times, magnitudes, etc.
...
# -i option to shuffle the columns, plus scaling the magintue (col 4) by 50 to
# yield a fake symbol size in km
gmt convert ${URL} -i2,1,3,4+s50,0 -hi1 > quakes.txt
# Make standard r,g,b seismicity depth color table
gmt makecpt -Cred,green,blue -T0,70,300,10000 > quakes.cpt
# Create a file with one record per day in 2018 in first column, with
